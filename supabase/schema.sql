...
-- Función para comprobar si es admin (SECURITY DEFINER para evitar recursión)
...
-- Función para crear perfil automáticamente al registrarse con rol default 'customer'
...
CREATE POLICY "Users can view own profile, admins view all" ON public.profiles FOR SELECT USING (auth.uid() = id OR is_admin());

DROP POLICY IF EXISTS "Users can update own profile" ON public.profiles;
CREATE POLICY "Users can update own profile" ON public.profiles FOR UPDATE USING (auth.uid() = id) WITH CHECK (auth.uid() = id);

DROP POLICY IF EXISTS "Only admins can change roles" ON public.profiles;
CREATE POLICY "Only admins can change roles" ON public.profiles FOR UPDATE USING (is_admin());
...
CREATE POLICY "Users can view own orders, admins view all" ON public.orders FOR SELECT USING (auth.uid() = user_id OR is_admin());
...
CREATE POLICY "Order items viewable through orders" ON public.order_items FOR SELECT USING (EXISTS (SELECT 1 FROM public.orders WHERE id = order_id AND (user_id = auth.uid() OR is_admin())));

DROP POLICY IF EXISTS "Users can insert items in own orders" ON public.order_items;
CREATE POLICY "Users can insert items in own orders" ON public.order_items FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.orders WHERE id = order_id AND user_id = auth.uid()));
